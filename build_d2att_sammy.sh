...
export PACKAGEDIR=$PARENT_DIR/Packages/TW_KK_ATT
#Enable FIPS mode
...
make VARIANT_DEFCONFIG=msm8960_m2_att_defconfig KT747_defconfig SELINUX_DEFCONFIG=selinux_defconfig
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	./mkbootimg --cmdline 'console=null androidboot.hardware=qcom user_debug=31' --kernel $PACKAGEDIR/zImage --ramdisk $PACKAGEDIR/ramdisk.gz --base 0x80200000 --pagesize 2048 --ramdiskaddr 0x82200000 --output $PACKAGEDIR/boot.img 
	export curdate=`date "+%m-%d-%Y"`
	cd $PACKAGEDIR
	cp -R ../META-INF .
	rm ramdisk.gz
	rm zImage
	rm ../KT747-TW-KK-4.4-ATT*.zip
	zip -r ../KT747-TW-KK-4.4-ATT-$curdate.zip .
	cd $KERNELDIR
else
